...
class Solution:
    def spirallyTraverse(self, matrix):
        def boundaryTraversal(matrix, startr, endr, startc, endc):
            a = []
            # to traverse to the right
            for col in range(startc, endc + 1):
                a.append(matrix[startr][col])
            # to traverse to the downward
        ...
        while startr <= endr and startc <= endc:
            res.extend(boundaryTraversal(matrix, startr, endr, startc, endc))
            startr += 1
            endr -= 1
            startc += 1
            endc -= 1
